import Supplier from '../../models/Product/supplier.model';
...
export async function updateSupplierById(req, res) {
    ...
    try {
        const supplierItem = new Supplier(SupplierData);
        await supplierItem.updateSupplier(SupplierId);
        return res.status(200).json({ message: 'Sửa nhà cung cấp thành công' });
